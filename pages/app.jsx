...
export default function App() {
  const [coin,setCoin]=useState('');const [result,setResult]=useState('');const [loading,setLoading]=useState(false);const [error,setError]=useState('');const [search,setSearch]=useState('');const [drawer,setDrawer]=useState(false);const [market,setMarket]=useState(null);const [fg,setFg]=useState(null);const [recent,setRecent]=useState([]);const [copied,setCopied]=useState(false);
  ...
  return (
    <div style={{minHeight:'100vh',background:'#080c14',color:'#f1f5f9',fontFamily:'system-ui,sans-serif'}}>
      ...
      {/* MARKET BAR */}
      <div style={{background:'#060a12',borderBottom:'1px solid #1a2332',padding:'0 12px',height:30,display:'flex',alignItems:'center',gap:12,overflowX:'auto',whiteSpace:'nowrap'}}>
      ...
      {/* TOPBAR */}
      <div style={{background:'#0d1421',borderBottom:'1px solid #1a2332',height:50,display:'flex',alignItems:'center',padding:'0 12px',gap:10,position:'sticky',top:30,zIndex:50}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,flex:1,minWidth:0}}>
            ...
            {result&&!loading&&(
              <div style={{marginLeft:'auto',display:'flex',gap:6,flexShrink:0}}>
                ...
                <a href="https://t.me/deeptradescan" target="_blank" style={{background:'rgba(37,175,254,0.1)',border:'1px solid rgba(37,175,254,0.25)',borderRadius:7,padding:'5px 10px',color:'#29A8EB',fontSize:11,fontWeight:600}}>✈️ TG</a>
              </div>
            )}
          </div>
        ):(
          <div style={{display:'flex',alignItems:'center',gap:10,flex:1}}>
            <img src="/logo.webp" style={{width:28,height:28,borderRadius:7,objectFit:'cover'}} alt="DTS"/>
            ...
            <div style={{marginLeft:'auto',display:'flex',gap:8,alignItems:'center'}}>
              <a href="https://t.me/deeptradescan" target="_blank" style={{background:'rgba(37,175,254,0.1)',border:'1px solid rgba(37,175,254,0.2)',borderRadius:7,padding:'5px 10px',color:'#29A8EB',fontSize:11,fontWeight:600}}>✈️ Telegram</a>
              <a href="https://twitter.com/deeptradescan" target="_blank" style={{background:'rgba(255,255,255,0.05)',border:'1px solid #1a2332',borderRadius:7,padding:'5px 10px',color:'#f1f5f9',fontSize:11,fontWeight:600}}>𝕏 Twitter</a>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{padding:'16px 14px 0'}}>
        ...
        {!coin&&!loading&&(
          <div style={{textAlign:'center',paddingTop:32}}>
            <img src="/logo.webp" style={{width:80,height:80,borderRadius:18,objectFit:'cover',marginBottom:16,boxShadow:'0 0 40px rgba(26,106,255,0.3)'}} alt="DTS"/>
            <div style={{fontSize:20,fontWeight:900,marginBottom:8,background:'linear-gradient(135deg,#60a5fa,#a78bfa)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>CHARTOS AI MODU</div>
            <div style={{fontSize:12,color:'#475569',marginBottom:24,lineHeight:1.8}}>SMC • ICT • Wyckoff • Volume Profile<br/>200+ coin için kurumsal yapay zeka analizi</div>
            <button onClick={()=>setDrawer(true)} style={{background:'linear-gradient(135deg,#2563eb,#8b5cf6)',borderRadius:12,padding:'13px 30px',color:'#fff',fontSize:15,fontWeight:800,boxShadow:'0 6px 24px rgba(37,99,235,0.4)',marginBottom:28,border:'none',cursor:'pointer'}}>
              🔱 Analiz Başlat
            </button>
            <div style={{display:'flex',gap:10,justifyContent:'center',marginBottom:24,flexWrap:'wrap'}}>
              <a href="https://t.me/deeptradescan" target="_blank" style={{display:'flex',alignItems:'center',gap:8,background:'rgba(37,175,254,0.08)',border:'1px solid rgba(37,175,254,0.2)',borderRadius:12,padding:'12px 18px',color:'#29A8EB',fontWeight:700,fontSize:13}}>
                <span style={{fontSize:20}}>✈️</span>
                <div style={{textAlign:'left'}}><div>Telegram Kanalı</div><div style={{fontSize:10,color:'rgba(41,168,235,0.6)',fontWeight:400}}>Canlı sinyaller • Ücretsiz</div></div>
              </a>
              <a href="https://twitter.com/deeptradescan" target="_blank" style={{display:'flex',alignItems:'center',gap:8,background:'rgba(255,255,255,0.03)',border:'1px solid #1a2332',borderRadius:12,padding:'12px 18px',color:'#f1f5f9',fontWeight:700,fontSize:13}}>
                <span style={{fontSize:18,fontWeight:900}}>𝕏</span>
                <div style={{textAlign:'left'}}><div>Twitter / X</div><div style={{fontSize:10,color:'#334155',fontWeight:400}}>Günlük analizler</div></div>
              </a>
            </div>
            {fgV&&(
              <div style={{maxWidth:240,margin:'0 auto 20px',background:'#0d1421',border:'1px solid #1a2332',borderRadius:12,padding:'16px'}}>
            ...
            {recent.length>0&&(
              <div style={{maxWidth:280,margin:'0 auto'}}>
                <div style={{fontSize:9,color:'#334155',letterSpacing:2,marginBottom:8}}>SON ANALİZLER</div>
                <div style={{background:'#0d1421',border:'1px solid #1a2332',borderRadius:12,overflow:'hidden'}}>
                  {recent.slice(0,5).map((h,i)=>(
                    <button key={i} onClick={()=>analyze(h.coin)} style={{width:'100%',display:'flex',justifyContent:'space-between',padding:'9px 14px',background:'transparent',borderTop:i?'1px solid #0f172a':'none',color:'#94a3b8',fontSize:12,textAlign:'left'}}
        ...
        {loading&&(
          <div style={{textAlign:'center',paddingTop:32}}>
            ...
            <div style={{fontSize:15,fontWeight:700,marginBottom:4}}>{coin} analiz ediliyor</div>
        ...
        {/* Analiz */}
        {result&&!loading&&(
          <div style={{background:'#0d1421',border:'1px solid #1a2332',borderRadius:12,padding:'18px 14px',lineHeight:1.8,wordBreak:'break-word'}}>
            {result.split('\n').map((line,i)=>{
              const t=line.trim();
              if(!t)return<br key={i}/>;
              if(t.includes('CHARTOS')&&(t.includes('ANALİZ')||t.includes('AI')))return<div key={i} style={{background:'linear-gradient(135deg,rgba(37,99,235,0.15),rgba(139,92,246,0.15))',border:'1px solid rgba(99,102,241,0.4)',borderRadius:10,padding:'14px',marginBottom:16,textAlign:'center',fontSize:15,fontWeight:900,color:'#fff'}}>{t}</div>;
              if(t.match(/^(PİYASA YAPISI|ANA SEVİYELER|SENARYO|YÜKSEK OLASILIKLI|YAPAY ZEKA İÇGÖRÜSÜ|AI İÇGÖRÜSÜ)/))return<div key={i} style={{borderLeft:'3px solid #2563eb',paddingLeft:10,marginTop:20,marginBottom:8,fontSize:11,fontWeight:800,color:'#60a5fa',letterSpacing:1.5,textTransform:'uppercase'}}>{t}</div>;
              if(t.match(/^(Varlık|Güncel Fiyat|24s|Ana Timeframe|AI Bias|Tanrısal Bias):/))return<div key={i} style={{display:'flex',gap:8,padding:'5px 0',borderBottom:'1px solid #0f172a',fontSize:12}}><span style={{color:'#475569',minWidth:120,flexShrink:0}}>{t.split(':')[0]}</span><span style={{color:'#f1f5f9',fontWeight:600}}>{t.split(':').slice(1).join(':').trim()}</span></div>;
              ...
              if(t.startsWith('Risk'))return<div key={i} style={{background:'rgba(239,68,68,0.07)',border:'1px solid rgba(239,68,68,0.2)',borderRadius:8,padding:'10px 12px',marginTop:16,color:'#fca5a5',fontSize:11}}>⚠️ {t}</div>;
      ...
      {/* FOOTER */}
      {!loading&&(
        <div style={{background:'#060a12',borderTop:'1px solid #1a2332',padding:'24px 16px',marginTop:32}}>
          <div style={{maxWidth:500,margin:'0 auto'}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
              ...
              <div>
                ...
                <div style={{fontSize:9,color:'#334155',letterSpacing:1}}>CHARTOS ENGINE v12.0</div>
              </div>
            </div>
            <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}>
              <a href="https://t.me/deeptradescan" target="_blank" style={{display:'flex',alignItems:'center',gap:6,background:'rgba(37,175,254,0.08)',border:'1px solid rgba(37,175,254,0.2)',borderRadius:8,padding:'7px 14px',color:'#29A8EB',fontSize:12,fontWeight:600}}>✈️ Telegram Kanalı</a>
              <a href="https://twitter.com/deeptradescan" target="_blank" style={{display:'flex',alignItems:'center',gap:6,background:'rgba(255,255,255,0.03)',border:'1px solid #1a2332',borderRadius:8,padding:'7px 14px',color:'#94a3b8',fontSize:12,fontWeight:600}}>𝕏 Twitter / X</a>
            </div>
            <div style={{display:'flex',justifyContent:'space-between',borderTop:'1px solid #0f172a',paddingTop:10,flexWrap:'wrap',gap:6}}>
              <span style={{fontSize:10,color:'#1e293b'}}>© 2026 Deep Trade Scan</span>
              <span style={{fontSize:10,color:'#1e293b'}}>⚠️ Finansal tavsiye değildir</span>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{position:'relative',width:280,height:'100%',background:'#0d1421',borderRight:'1px solid #1a2332',display:'flex',flexDirection:'column',animation:'slide .22s ease',zIndex:1}}>
            <div style={{padding:'16px 14px',borderBottom:'1px solid #1a2332',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                <img src="/logo.webp" style={{width:30,height:30,borderRadius:8,objectFit:'cover'}} alt="DTS"/>
                <div>
                  <div style={{fontSize:12,fontWeight:800}}>DEEP TRADE SCAN</div>
            ...
            <div style={{padding:'10px 12px',borderBottom:'1px solid #1a2332',display:'flex',gap:6,flexShrink:0}}>
              <a href="https://t.me/deeptradescan" target="_blank" style={{flex:1,textAlign:'center',background:'rgba(37,175,254,0.08)',border:'1px solid rgba(37,175,254,0.2)',borderRadius:7,padding:'6px',color:'#29A8EB',fontSize:11,fontWeight:600}}>✈️ Telegram</a>
              <a href="https://twitter.com/deeptradescan" target="_blank" style={{flex:1,textAlign:'center',background:'rgba(255,255,255,0.03)',border:'1px solid #1a2332',borderRadius:7,padding:'6px',color:'#94a3b8',fontSize:11,fontWeight:600}}>𝕏 Twitter</a>
            </div>
            <div style={{padding:'10px 12px 6px',flexShrink:0}}>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Coin ara..." style={{width:'100%',padding:'9px 12px',background:'#111827',border:'1px solid #1a2332',borderRadius:9,color:'#f1f5f9',fontSize:13}}/>
            </div>
            ...
            <div style={{padding:'10px 14px',borderTop:'1px solid #1a2332',fontSize:10,color:'#1e293b',display:'flex',justifyContent:'space-between',flexShrink:0}}>
              <span>deeptradescan.com</span><span>v12.0</span>
            </div>
          </div>
        </div>
      )}
